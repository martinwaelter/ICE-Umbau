"""REV6: preserve the previous artifact; reuse only inspected layout/chapters."""
...
prefix = prefix.replace("ICE_2976_Umbauanleitung_REV5.pdf", "ICE_2976_Umbauanleitung_REV6_PRUEFFASSUNG.pdf")
...
SOURCES[19]=('Märklin','CAN-Protokoll 2.0','Abschnitt 8.1.3: lokomotive.cs2 und mfxuid; UID ist nicht die zugewiesene Fahradresse','https://streaming.maerklin.de/public-media/cs2/cs2CAN-Protokoll-2_0.pdf')
...
add('G = Prüftor. Offen bedeutet nicht widerlegt, aber auch nicht ausführungsreif. Diese Fassung ersetzt die entsprechenden Arbeitsanweisungen der REV5. Frühere Variantenpläne nicht parallel anwenden. Im REV6-Zielaufbau sind beide Schleifer über RT verbunden; Decoder-U+ und Funktionsausgänge bleiben getrennt.')
...
warn('G0-STOPP: zentrale Lücke bleibt offen','Die exakte Zuordnung der Märklin-mfx-UID zu den beiden ESU-Eingabefeldern ist in den ausgewerteten Primärquellen nicht nachgewiesen. Hier endet die selbst ausführbare Einrichtung. Keine Werte ausprobieren. Erst belastbare Hersteller-/Fachbetriebsbestätigung plus realer Kombinationstest ermöglichen die Fortsetzung.')
...
step('C1','Testumgebung vorbereiten.','Automatische Fahrereignisse für die Testgeräte deaktivieren, Regler auf 0. CS3-Ausgang von der Anlage trennen. Keine Gleisanschlussbox, Mobile-Station-Einspeisung oder Programmerausgänge an denselben Aufbau anschließen. Beide Decoder behalten getrennte Ausgangslasten.')
...
('C3: Abschalten; Slave ergänzen','Beide Eingänge an derselben Quelle. Nur der gemeinsame ICE-Eintrag wird bedient. Kein neu entstandener zweiter unabhängig zu bedienender Zug. Vorbestehende Einträge nicht blind löschen.'),
...
('C6: Versorgung neu starten','Nach geordnetem Ausschalten beide gemeinsam neu starten und C4 wiederholen. Danach Slave stromlos entfernen, wieder stromlos ergänzen und erneut testen.'),
('C7: Neue mfx-Adressvergabe','Fachkundig mit gesicherter Testumgebung oder separater Testzentrale prüfen. Keine produktive CS3-Lokliste für diesen Test löschen. Ohne diesen Versuch bleibt automatische Neuanmeldung ungeprüft.'),
...
add('Eine vollständige isolierende Befestigung kann fachkundig ausgelegt werden, ist aber hier noch kein fertiger Ausweichplan: Schraubenschaft, Ring, Unterseite und Nachbarflächen müssen isoliert bleiben; ein erforderlicher Radmassepfad darf nicht verloren gehen. Nur eine Kunststoff-Unterlegscheibe oben reicht nicht. Keine unbestimmte Halter-Artikelnummer kaufen.')
refs(1,3)
...
story.append(photo('/Users/martinwaelter/ICE Umbau/Arbeitsstand_REV10/bilder/ice2976_60977-p5-pads.jpg',W,300))
add('Originalzeichnung aus Märklins 60977-Anleitung, S. 5, unverändert. Kein Foto des vorhandenen Trägers. Die links gezeichnete Buchse SUSI und die darunter liegende Lautsprecherbuchse sind getrennte Anschlüsse.<super>1</super>','small')
...
step('46','60977 für den echten Motor vorbereiten.','Für den eingebauten 60941-HLA passenden Motortyp anhand der Märklin-Unterlagen wählen. Prüfstand-Motoreinstellungen nicht blind übernehmen. ICE-Soundprojekt mit Märklin-Werkzeug einrichten, leise starten, mfx aktiv lassen. Keine automatische Einmessfahrt auf dem kurzen Prüfgleis.')
...
step('49','Einstellungen wieder auslesen.','Geschriebene Identität, Sonderoption und Mapping mit dem gespeicherten Prüfprojekt vergleichen. Bei Abweichung die Verbindung nicht unter Spannung ausprobieren. Programmerzugang wird auch für spätere Korrekturen benötigt.')
step('50','Endgültige Funktionstabelle abnehmen.','F0 aus: beide Köpfe dunkel. F0 vorwärts: Motorseite weiß, Dummy rot. F0 rückwärts: Motorseite rot, Dummy weiß. Alle Zustände im Stand, Innenlicht und Sound unabhängig. Erst danach langsame reale Fahrtrichtung prüfen.')
...
('Dieselben isoliert geprüften Messpaare erneut messen.','Die vollständigen Vorproben, Isolationsmessungen und Nachproben nach 6a mit unveränderten Fahrzeugclips wiederholen. Dieselben passiven Pfade erneut messen.'),
...
page('E. Änderungen und verbleibender Prüfstatus')
table(['Befund aus den Zwischenberichten','Behandlung in REV6','Status'],[
('Synchronisation erst nach Gesamtumbau','Kapitel A-C mit Ausstattung, Einrichtungsgrenze und Testfällen vorgezogen','Ablauf korrigiert; G0 real offen.'),
('ESU-Programmierzugang unklar','53451, Windows und sichere Prüfaufnahmen explizit erfasst','Bestand/Zugang noch zu bestätigen.'),
('Einmessfahrt beim Firmwarenachweis','Lesen von Schreiben getrennt; Aktivierungswert und Fahrbefehl-Warnung an der kritischen Stelle','Text korrigiert.'),
('LoDi-Pads / VCC fehlen','9a mit tatsächlichen Herstellerbezeichnungen, Träger +Ub/+5V/GND getrennt','Eigene Revision offen.'),
('Befestigung nicht ausführbar freigegeben','9 mit Ringprotokoll und ausdrücklichem G1-STOPP','Keine erfundene Halterfreigabe.'),
('Lichtfarbe zu Ausgang unbestimmt','12a mit einer festgelegten Mapping-/Verdrahtungsvariante','Am ESU und Zug zu prüfen.'),
('RT/GE erst nach Löten geklärt','14-14b: unabhängige Identifikation vor Löten; passive und bestückte Prüfung getrennt','Arbeitsfolge korrigiert.'),
('Messkontakt kann falsches OL erzeugen','6/6a mit festen Hilfsclips und positiven Vor-/Nachproben','Methodik korrigiert; Fahrzeugtest offen.'),
('R3/R6 und Helligkeitsbezug','11a trennt Code, Strompfad und optische Abstimmung','Werte nicht freigegeben.'),
('Lautsprecher-Steckerübergang fehlt','9b mit reversibler Adapterstrategie und Prüfablauf','Steckertyp noch offen.')],[2.1,2.9,1.7])
add('Die Gegenprüfung umfasst drei fokussierte Arbeitsbereiche: mfx-Architektur, Anschluss/Befestigung und Messlogik. Sie ist eine Quellen-/Entwurfsprüfung, keine dreifache Hardwareabnahme. Die dokumentierten STOPP-Punkte werden durch einen fehlerfreien PDF-Export nicht aufgehoben.')
...
add('Der ursprüngliche Direktlink zur 60977-Anleitung liefert beim Abruf teilweise HTTP 403. Die lokale Herstellerkopie ist vorhanden; insbesondere S. 4/5/7 wurden für Stecker, Pads und Einmessfahrt herangezogen. Fehlende Web-Erreichbarkeit ist kein Ersatz für die Prüfung des tatsächlich beiliegenden Dokuments.','small')
for n in range(1,11):
...
warn('Verbindlicher Dokumentstatus','REV6 ist eine Prüffassung, nicht das Versprechen eines vollständig risikofreien Umbaus. Ohne G0, reale Platinenzuordnung und bestandene Einzelprüfungen keine Gesamtfreigabe. Keine erfundenen Messwerte, Hardwaretests oder vollständigen externen Konsilvoten sind in diesem Dokument enthalten.')
...
def foot(c,doc):
    ...
    c.drawString(42,24,'REV6 - Prüffassung; offene Prüftore beachten')
    c.drawRightString(A4[0]-42,24,str(doc.page));c.restoreState()
doc=ManualDoc(str(OUT),pagesize=A4,rightMargin=42,leftMargin=42,topMargin=40,bottomMargin=40,title='ICE 2976 - Umbauanleitung REV6 Prüffassung',author='Technische Prüf- und Arbeitsanleitung',subject='Vorgezogener mfx-Nachweis, Anschlussklärung, Motorisolation, Kupplungsprüfungen')
doc.build(story,onFirstPage=foot,onLaterPages=foot)
print(OUT)
print('Kapitel:',len(sections))
