...
class Layout:
 def __init__(self,out,pages):
  ...
  self.c.setTitle('ICE 2976 - REV15 | Kompakte LoDi-Umbauanleitung')
 ...
 def build(self):
  for i,p in enumerate(self.pages,1):
   ...
   if y<50:raise ValueError(f'Page {i} {p["title"]}: overflow by {50-y:.1f}pt; bottom {y:.1f}')
   c.setStrokeColor(LINE);c.setLineWidth(.5);c.line(M,48,W-M,48)
   c.setFont('Body',7.7);c.setFillColor(GREY)
   c.drawString(M,35,'REV15 | ICE 2976 | LoDi-512 / LoDi-511 / 2x LoDi-514')
