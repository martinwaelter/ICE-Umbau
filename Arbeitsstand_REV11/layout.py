...
class Layout:
 def __init__(self,out,pages):
  ...
  self.c.setTitle('ICE 2976 - REV11 | Bebilderte Werkstattanleitung ohne Händler')
 ...
 def build(self):
  for i,p in enumerate(self.pages,1):
   ...
   c.drawString(M,35,'REV11 | ICE 2976 | 11.09.2026')
   c.drawRightString(W-M,35,f'{i} / {len(self.pages)}')
   refs=p.get('sources',[])
   c.drawString(M,23,'Quellen: '+', '.join(refs)+' (S. 40)' if refs else 'Originalreferenz: REV10 | Persönliche Gerätewerte vor Anwendung eintragen')
   if i<len(self.pages):
